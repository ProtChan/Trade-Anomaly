#!/usr/bin/env python3
"""Build a rolling USDJPY H1 midpoint dataset from Dukascopy quote bars and recent ticks."""
...
def load_ticks(path: Path) -> tuple[dict[int, dict[str, float | int]], list[float], int]:
    """Aggregate raw Bid/Ask ticks into UTC H1 bars of tick-by-tick midpoint."""
    if not path.exists() or path.stat().st_size == 0:
...
def main() -> None:
    ...
    ap.add_argument("--full-file", action="append", default=[], help="Repeat for Dukascopy full Bid/Ask H1 CSVs")
    ap.add_argument("--tick-file", action="append", default=[], help="Repeat for recent Dukascopy tick CSVs; tick-derived H1 overrides matching H1 rows")
    ...
    if not args.full_file and not args.tick_file:
        raise SystemExit("At least one --full-file or --tick-file is required")
    ...
    for filename in args.tick_file:
        path = Path(filename)
        fresh, spreads, ticks = load_ticks(path)
        merged.update(fresh)
        fresh_count += len(fresh)
        raw_tick_count += ticks
        all_spreads.extend(spreads)
        tick_sources.append(path.name)
        sources.append(path.name)
    ...
    if tick_sources:
        source = "Dukascopy Bid+Ask H1 + raw Bid/Ask ticks → Mid H1"
        method = "Historical H1 uses Dukascopy full quote midpoint; recent H1 is aggregated from raw ticks with mid=(bid+ask)/2 per tick; screener entry/exit uses mid_open"
    else:
        source = "Dukascopy full Bid+Ask H1 → Mid"
        method = "Dukascopy full quote bars; fieldwise midpoint = (bid + ask) / 2; screener entry/exit uses mid_open"
    ...
    info = {
        ...
        "tick_sources_this_run": tick_sources,
        "bars": len(rows),
        ...
        "raw_ticks_loaded_this_run": raw_tick_count,
